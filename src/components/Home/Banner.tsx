      ...
      {bannerCnt.map((cnt, index) => (
        <div key={index}>
          ...
          <div className="relative text-primary z-10 flex flex-col justify-center items-center h-full text-center">
            <p
              className={`absolute card-glass text-center mt-[50%] text-5xl font-bold leading-tight mb-4 max-sm:text-lg max-sm:mb-1 transition-opacity duration-1000 ease-in-out ${
                index === currentImageIndex ? "opacity-100" : "opacity-0"
              }`}
            >
              {cnt.text}
